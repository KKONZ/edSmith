...
The episodic memory tree needs a root node (Session 0) representing the baseline performance this system aims to improve upon. The question is whether Session 0 uses metrics cited directly from Nguyen et al. (2026) or metrics from an internally-run replication.

We replicate the relevant Nguyen et al. modeling techniques ourselves on the same dataset, record the results, and write them as Session 0 in the episodic memory tree.
