...
- ✅ **Commit 12:** Add `src/edsmith/tools/mcp/tools.py` — `register_grammar_check`, `register_aoa_stats`, `register_complexity_stats`, `register_discourse_analysis` following RocketSmith `register_*(app: FastMCP)` pattern. Export from `tools/mcp/__init__.py`. Register all four in `src/edsmith/mcp/__main__.py`. Add `docs/adr/0012-linguistic-feature-tools.md`.
...
- ✅ **Commit 16b:** Rewrite `run_diagnostic` in `src/edsmith/chief_examiner/diagnostic.py`. Replace `_select_mode` / UCB1 with `_load_iteration_history` — loads prior `proposals/iter{n}.json` and `metrics_iter{n}.json` per iteration, formats as "Iteration N: val accuracy=X qwk=Y smd=Z | diagnosis: ... | proposed: ... | human: approved/rejected". Remove `_ucb1`, `_select_mode`, `EpisodicMemory` import and parameter. Signature now takes `drive_path: Path`. System prompt instructs Chief Examiner to use history to avoid repeating unhelpful changes and to honour human critiques.
