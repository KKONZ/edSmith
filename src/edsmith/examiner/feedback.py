...
def _build_messages(
    ...
    if strategy.use_tool_calling:
        ...
        system_parts.append(
            ...
            "- When `grammar_check` flags a word and its `aoa` field is null, call "
            "`aoa_lookup` with that word. If it is not in the AoA vocabulary, the result "
            "will include `close_matches` — a short list of spelling-similar words with "
            "their AoA values. Use the essay context to pick whichever candidate the "
            "writer most plausibly intended (prefer the one whose vocabulary level fits "
            "the surrounding text). Report the misspelling, your chosen intended word, "
            "and its AoA in your feedback.\n"
